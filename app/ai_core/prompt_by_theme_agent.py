...
def cool_prompt(users_theme: str, desires: str = None) -> str:
    ...
    курсы для обучения по заданной тебе теме. Твоя задача по теме и пожеланиям, которые тебе говорит пользователь,
    составить промпт
    ...
    Тема пользователя: {users_theme}. Пожелания пользователя: {desires} Не используй md в своем ответе, ты пишешь для
    LLM."""
